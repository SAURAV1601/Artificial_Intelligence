...
# 2. Crossover (PMX - partially matched cross over method)
...
while flag:
	...
	for i in range(0,crosslen/2):
		...
		child1=[0]*cities
		for k in range(randCrossOverDivision[0],randCrossOverDivision[1]+1):
			child1[k]=parent1[k]
		# Following is the PMX cross over method where elements of other parent are inherited to child based on partial matches
		for k in range(randCrossOverDivision[0],randCrossOverDivision[1]+1):
			if not parent2[k] in child1:
				ele=parent1[k]
				idx=parent2.index(ele)
				if idx<randCrossOverDivision[0] or idx>randCrossOverDivision[1]:
					child1[idx]=parent2[k]
					...
					while loopFlag and loopCount<(randCrossOverDivision[1]-randCrossOverDivision[0]):
						element=parent1[inneridx]
						inneridx=parent2.index(element)
						loopCount+=1
						if child1[inneridx]==0 and (inneridx<randCrossOverDivision[0] or inneridx>=randCrossOverDivision[1]):
							child1[inneridx]=parent2[k]
							loopFlag=0
		# adding elements to child from other parent which don't yet exist in child
		...
		for k in range(randCrossOverDivision[0],randCrossOverDivision[1]+1):
			if not parent1[k] in child2:
				ele=parent2[k]
				idx=parent1.index(ele)
				if idx<randCrossOverDivision[0] or idx>randCrossOverDivision[1]:
					child2[idx]=parent1[k]
				else:
					loopFlag=1
					inneridx=idx
					loopCount=0
					...
					while loopFlag and loopCount<(randCrossOverDivision[1]-randCrossOverDivision[0]):
						element=parent2[inneridx]
						inneridx=parent1.index(element)
						loopCount+=1
						if child2[inneridx]==0 and (inneridx<randCrossOverDivision[0] or inneridx>=randCrossOverDivision[1]):
							child2[inneridx]=parent1[k]
							loopFlag=0
